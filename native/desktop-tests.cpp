...
struct Child {
    ...
    void preview() {moveTo(center());delay(30);command(Preview);check(until([&]{return query(window,1)!=0;},1200),"System cursor replacement becomes active");delay(180);}
...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        auto root=std::filesystem::path(executablePath()).parent_path();auto directory=root/L"desktop-checks";
        ...
        cleanup.changed=true;
        {
            ...
            check(query(child.window,9)>0&&query(child.window,5)>sampleStart,"Raw Input receives motion and samples positions");
            ...
            child.command(OpenSettings);HWND dialog=reinterpret_cast<HWND>(query(child.window,10));check(dialog&&IsWindowVisible(dialog),"Native settings dialog opens");
            ...
            child.command(OpenSettings);dialog=reinterpret_cast<HWND>(query(child.window,10));SendMessageW(dialog,WM_COMMAND,IDC_DEFAULTS,0);SendMessageW(dialog,WM_COMMAND,IDOK,0);delay(100);
