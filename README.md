...
当前源码版本 **0.4.3**，使用 Rust + Win32。普通用户权限运行，驻留系统托盘，无需网络、账户或额外运行时。源码采用 [MIT 许可证](LICENSE)。
...
0.4.3 修复跨过分栏等应用私有光标时丢失甩动触发、打断放大效果的问题。私有光标保持原样，手势和效果计时继续，回到标准光标时直接显示当前放大帧。验证与占用见 [分栏修复记录](validation/SPLITTER-FIX-0.4.3.md)。
...
放大标准系统光标；应用私有光标保持原样，但不会清空甩动手势或丢弃效果。效果有效期内回到标准光标即显示放大帧；如果一直停留在私有光标区域，该图标本身不会变大。最大高度 256 物理像素，高 DPI 或高倍率下可能达不到所选倍率；屏幕右侧和底部会镜像箭身，热点仍对应真实点击位置。
